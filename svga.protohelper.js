...
var SVGAProtoHelper = function () {
    function SVGAProtoHelper() {
        _classCallCheck(this, SVGAProtoHelper);
    }

    _createClass(SVGAProtoHelper, null, [{
        key: "convertToProto",
        value: function convertToProto(obj) {
            var movieItem = new Proto.MovieEntity();
            movieItem.setVersion(obj.ver);
            var movieParams = new Proto.MovieParams();
            ...
            var imagesMap = movieItem.getImagesMap();
            for (var aKey in obj.images) {
                var aValue = obj.images[aKey];
                imagesMap.set(aKey, aValue);
            }
            var spritesList = movieItem.getSpritesList();
            obj.sprites.forEach(function (sprite) {
                var spriteItem = new Proto.SpriteEntity();
                spriteItem.setImagekey(sprite.imageKey);
                var framesList = spriteItem.getFramesList();
                sprite.frames.forEach(function (frame) {
                    var frameItem = new Proto.FrameEntity();
                    if (typeof frame.alpha === "number") {
                        frameItem.setAlpha(frame.alpha || 1.0);
                    }
                    if (_typeof(frame.layout) === "object") {
                        frameItem.setLayout(function (frameLayout) {
                            var layout = new Proto.Layout();
                    ...
                    if (_typeof(frame.transform) === "object") {
                        frameItem.setTransform(function (frameTransform) {
                    ...
                    if (_typeof(frame.shapes) === "object") {
                        var shapesList = frameItem.getShapesList();
                        frame.shapes.forEach(function (shape) {
                            var shapeItem = new Proto.ShapeEntity();
                            shapeItem.setType(function () {
                                if (shape.type === "shape") {
                            ...
                            if (_typeof(shape.args) === "object") {
                                if (shape.type === "shape") {
                                    var args = new Proto.ShapeEntity.ShapeArgs();
                                    if (typeof shape.args.d === "string") {
                                        args.setD(shape.args.d);
                                    }
                                    shapeItem.setShape(args);
                                } else if (shape.type === "rect") {
                                    var _args = new Proto.ShapeEntity.RectArgs();
                                    if (typeof shape.args.x === "number") {
                                        _args.setX(shape.args.x);
                                    }
                                    if (typeof shape.args.y === "number") {
                                        _args.setY(shape.args.y);
                                    }
                                    if (typeof shape.args.width === "number") {
                                        _args.setWidth(shape.args.width);
                                    }
                                    if (typeof shape.args.height === "number") {
                                        _args.setHeight(shape.args.height);
                                    }
                                    if (typeof shape.args.cornerRadius === "number") {
                                        _args.setCornerradius(shape.args.cornerRadius);
                                    }
                                    shapeItem.setRect(_args);
                                } else if (shape.type === "ellipse") {
                                    var _args2 = new Proto.ShapeEntity.EllipseArgs();
                                    if (typeof shape.args.x === "number") {
                                        _args2.setX(shape.args.x);
                                    }
                                    if (typeof shape.args.y === "number") {
                                        _args2.setY(shape.args.y);
                                    }
                                    if (typeof shape.args.radiusX === "number") {
                                        _args2.setRadiusx(shape.args.radiusX);
                                    }
                                    if (typeof shape.args.radiusY === "number") {
                                        _args2.setRadiusy(shape.args.radiusY);
                                    }
                                    shapeItem.setEllipse(_args2);
                                }
                            }
                            if (_typeof(shape.styles) === "object") {
                                var styleItem = new Proto.ShapeEntity.ShapeStyle();
                                if (_typeof(shape.styles.fill) === "object") {
                                    var color = new Proto.ShapeEntity.ShapeStyle.RGBAColor();
                                ...
                                if (_typeof(shape.styles.stroke) === "object") {
                                    var _color = new Proto.ShapeEntity.ShapeStyle.RGBAColor();
                                    _color.setR(shape.styles.stroke[0]);
                                    _color.setG(shape.styles.stroke[1]);
                                    _color.setB(shape.styles.stroke[2]);
                                    _color.setA(shape.styles.stroke[3]);
                                    styleItem.setStroke(_color);
                                }
                                ...
                                if (_typeof(shape.styles.lineDash) === "object") {
                            ...
                            if (_typeof(shape.transform) === "object") {
                                shapeItem.setTransform(function (shapeTransform) {
                                    var transform = new Proto.Transform();
    ...
    return SVGAProtoHelper;
}();

if ((typeof window === "undefined" ? "undefined" : _typeof(window)) === "object") {
    window.SVGAProtoHelper = SVGAProtoHelper;
}

exports.SVGAProtoHelper = SVGAProtoHelper;
